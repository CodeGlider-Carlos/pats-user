...
<head>
    ...
    <style>
        ...
        *::after {
        ...
        .cc-card.brand-visa .cc-card__front {
            background: linear-gradient(135deg, #1a1f71, #2558e0);
        }

        .cc-card.brand-visa .cc-card__back {
            background: linear-gradient(135deg, #1a1f71, #2558e0);
        }

        .cc-card.brand-mc .cc-card__front {
            background: linear-gradient(135deg, #eb001b, #f79e1b);
        }

        .cc-card.brand-mc .cc-card__back {
            background: linear-gradient(135deg, #cc0016, #e08a10);
        }

        .cc-card.brand-amex .cc-card__front {
            background: linear-gradient(135deg, #007bc1, #00a0e3);
        }

        .cc-card.brand-amex .cc-card__back {
            background: linear-gradient(135deg, #006aab, #0090cc);
        }
...
<body>
    ...
    <header class="topbar">
        <div class="topbar__brand">
            ...
            PATS
            <span class="topbar__tag">· Red de franquicias</span>
        </div>
    ...
    <div class="sol-layout">
        ...
        <aside class="sidebar">
            ...
            <div class="sidebar__hero">
                ...
                <div class="sidebar__title">Solicitud de Alta<br>como Franquicia</div>
            ...
            <div class="sidebar__steps">
                ...
                @php
                    $stepsData = [
                        ...
                        ['label' => 'Franquicia', 'desc' => 'Perfil del titular'],
        ...
        (function() {
            ...
            const STEP_META = [{
                ...
                },
                {
                    ...
                    desc: 'Selfie, contrato firmado, firma digital y aceptación.'
                },
                {
                    tag: 'Paso 6 de 6',
            ...
            function bindModalidad() {
                ...
                [$('plazo_meses'), $('fecha_primer_vencimiento')].forEach(el => {
                    el?.addEventListener('change', buildPlan);
                });
            }
            ...
            function bindSubmit() {
                @php
                    $intentUrl     = route('franq.stripe.intent');
                    $submitUrl     = route('franq.publico.guardar');
                    $preValidarUrl = route('franq.publico.pre-validar');
                @endphp

                // Estado del pago — persiste entre reintentos para no cobrar dos veces
                let confirmedPaymentId = null;
                // Clave única por carga de página para idempotencia de red en Stripe
                ...
                $('frmDist')?.addEventListener('submit', async e => {
                    ...
                    try {
                        const modalidad  = document.querySelector('input[name=modalidad_pago]:checked')?.value || 'CONTADO';
                        const plazoMeses = parseInt(document.getElementById('plazo_meses')?.value || '0');

                        if (!confirmedPaymentId) {
                            // ── Paso 0: Pre-validar antes de cobrar ────────────
                            ...
                            if (!intentData.ok) throw new Error(intentData.error || 'No fue posible iniciar el pago.');
                            ...
                            if (modalidad === 'DIFERIDO' && plazoMeses > 0) {
                                confirmOpts.payment_method_options = {
                                    card: {
                                        installments: {
                                            plan: { count: plazoMeses, interval: 'month', type: 'fixed_count' },
                                        },
                                    },
                                };
                            }
                            ...
                            if (intentData.split) {
                                // ── Cobro dividido en dos transacciones ────────
                                btn.innerHTML = '<i class="mdi mdi-loading mdi-spin"></i> Procesando cobro 1 de 2...';
                                const { paymentIntent: pi1, error: err1 } = await stripe.confirmCardPayment(
                                    intentData.client_secret_1, confirmOpts
                                );
                                if (err1) throw new Error(err1.message || 'Primer cobro rechazado por el banco.');
                                if (pi1.status !== 'succeeded') throw new Error('El primer cobro no fue confirmado.');

                                btn.innerHTML = '<i class="mdi mdi-loading mdi-spin"></i> Procesando cobro 2 de 2...';
                                const { paymentIntent: pi2, error: err2 } = await stripe.confirmCardPayment(
                                    intentData.client_secret_2, confirmOpts
                                );
                                if (err2) throw new Error('El primer cobro fue exitoso. ' + (err2.message || 'El segundo cobro falló — contacta a soporte.'));
                                if (pi2.status !== 'succeeded') throw new Error('El segundo cobro no fue confirmado — contacta a soporte.');
                                ...
                                const { paymentIntent, error } = await stripe.confirmCardPayment(
                                    intentData.client_secret, confirmOpts
                                );
                                if (error) throw new Error(error.message || 'Pago rechazado por el banco.');
                                if (paymentIntent.status !== 'succeeded') throw new Error('El pago no fue confirmado.');

                                confirmedPaymentId = paymentIntent.id;
                            }
                        } else {
                            // Pago ya confirmado — solo reintentar el envío de datos
                            btn.innerHTML = '<i class="mdi mdi-loading mdi-spin"></i> Reintentando envío...';
                        }

                        // ── Paso 3: Guardar solicitud ──────────────────────────
                        ...
                        window.location.href = '{{ route("franq.publico.confirmacion") }}?' + params.toString();

                    } catch (err) {
                        toast(err.message || 'No fue posible enviar la solicitud.', 'error', 5000);
                        btn.disabled = false;
                        // Si el pago ya fue confirmado, avisar al usuario que no se volverá a cobrar
            ...
            function bindSelfie() {
                ...
                btnStart?.addEventListener('click', async () => {
                    try {
                        ...
                        toast('No se pudo acceder a la cámara. Verifica los permisos del dispositivo.', 'error');
                    }
                });
                ...
                btnTake?.addEventListener('click', () => {
                    ...
                    if (captura) { captura.src = data; captura.style.display = ''; }
                ...
                btnRetake?.addEventListener('click', () => {
                    ...
                    if (captura) { captura.src = ''; captura.style.display = 'none'; }
            ...
            function bindSignature() {
                ...
                function draw(e) {
                    ...
                    ctx.beginPath();
                    ctx.moveTo(pos.x, pos.y);
                    if (!hasSigned) { hasSigned = true; wrap?.classList.add('signed'); }
                }
            ...
            function updatePreviewDist() {
                ...
                if (pdSelfie) {
                    pdSelfie.innerHTML = selfie
                        ? `<img src="${selfie}" class="pd-img" alt="Selfie">`
                        : '<span class="pd-empty">Sin captura</span>';
                }
                if (pdFirma) {
                    pdFirma.innerHTML = firma
            ...
            function bindCard() {
                ...
                // Actualiza nombre en la tarjeta decorativa
